...
def rule_R3(C):
    ...
    for beta in C.nodes:
        for alpha in C.adj(beta):
            if not (C.is_arrow(beta, alpha) and C.is_star(alpha, beta)): 
                continue

            for gamma in C.adj(beta):
                if gamma == alpha:
                    continue
                if not (C.is_arrow(beta, gamma) and C.is_star(gamma, beta)): 
                    continue
...
def rule_R6(C):
    ...
    for beta in C.nodes:
        for alpha in C.adj(beta):
            if not (C.is_tail(alpha, beta) and C.is_tail(beta, alpha)):
                continue
            for gamma in C.adj(beta):
                if gamma == alpha:
                    continue
                if C.is_o(beta, gamma) and C.is_star(gamma, beta):
...
def rule_R8(C):
    ...
    for alpha in C.nodes:
        for gamma in C.adj(alpha):
            if not (C.is_o(alpha, gamma) and C.is_arrow(gamma, alpha)):
                continue
            ...
            for beta in C.nodes:
                ...
                cond1 = (
                    C.has_edge(alpha, beta)
                    and C.is_tail(alpha, beta)
                    and C.is_arrow(beta, alpha)
                )
                cond2 = (
                    C.has_edge(alpha, beta)
                    and C.is_tail(alpha, beta)
                    and C.is_tail(beta, alpha)
                )
...
def apply_R1_to_R10(C, sepset, callback=None):
